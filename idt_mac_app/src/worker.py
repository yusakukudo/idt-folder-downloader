...
from src.crawler import IDTCrawler
from src.downloader import IDTDownloader


class IDTWorkerThread(QThread):
    # Signals to communicate with the GUI thread
    crawl_progress = Signal(int, int, str)       # visited, files_count, current_filepath
    crawl_finished = Signal(str, list)           # target_dir, files_list
    file_progress = Signal(str, int, int, str)   # file_path, bytes_dl, total_bytes, status
    overall_progress = Signal(int, int, int, float, float) # completed_files, total_files, total_bytes, speed_bps, elapsed_sec
    finished_signal = Signal(bool, str)         # success, message
    log_signal = Signal(str)                     # log_text
    ...
    def run(self):
        try:
            ...
            self.downloader = IDTDownloader(concurrency=self.concurrency)

            if self._is_cancelled:
                self.downloader.cancel()

            # Run asyncio downloader loop in this thread
